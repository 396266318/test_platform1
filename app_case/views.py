...
def edit_case(request, cid):
    """编辑用例"""
    return render(request, "case/eidt.html")
...
def send_case(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse({"code": 10102, "message": "Header格式错误, 必须是标准JSON格式!"})

        try:
            per_value = json.loads(per_value)
        except json.decoder.JSONDecodeError:
            return JsonResponse({"code": 10103, "message": "参数格式错误,必须输标准JSON格式!"})
...
def get_case_info(request):
    ...
    if request.method == "POST":
        ...
        return JsonResponse({"code": 10200, "message": "success", "data": case_info})
    else:
        return JsonResponse({"code": 10100, "message": "请求方法错误"})
